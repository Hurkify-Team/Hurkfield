...
def create_survey(
    ...
    with get_conn() as conn:
        ...
        cur.execute(
            """
            INSERT INTO surveys
              (facility_id, template_id, survey_type, enumerator_name, status, created_at, created_by, updated_at, source)
            VALUES (?, ?, ?, ?, 'DRAFT', ?, ?, ?, ?)
            """,
            (
                int(facility_id),
                ...
                enumerator_name,
                _now_iso(),
                (created_by or enumerator_name or "System").strip(),
                _now_iso(),
