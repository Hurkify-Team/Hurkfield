...
def _print_templates(rows):
    ...
    print("-" * 90)
    for row in rows:
        tid, name, desc, created_at = row[:4]
        created_by = row[4] if len(row) > 4 else ""
        source = row[6] if len(row) > 6 else ""
        by_txt = f" | {created_by}" if created_by else ""
        src_txt = f" | {source}" if source else ""
        print(f"#{tid} | {name} | {desc or '-'} | {created_at}{by_txt}{src_txt}")
    print("-" * 90)
...
def _print_template_questions(rows):
    ...
    for row in rows:
        ...
        qtype = row[2] if len(row) > 2 else ""
        order = row[3] if len(row) > 3 else ""
        req = row[4] if len(row) > 4 else 0
        print(
